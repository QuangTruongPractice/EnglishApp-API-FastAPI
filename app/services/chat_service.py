...
class ChatService:
    def __init__(self):
        # Khởi tạo client Groq (cả bản sync và async)
    ...
    def transcribe_audio_groq(self, file_path: str) -> str:
        # Dùng Groq để chuyển giọng nói trong file thành văn bản
        try:
    ...
    def reset_chat_history(self, user_id: str, db: Session) -> bool:
        # Xóa hết lịch sử chat cũ của người dùng trong DB
        try:
    ...
    async def get_groq_chat_response(self, user_id: str, user_text: str, db: Session) -> str:
        # Lấy phản hồi từ AI, có tính tới 10 tin nhắn gần nhất để giữ ngữ cảnh
    ...
    async def generate_audio_file(self, text: str, request) -> Optional[str]:
        # Chuyển văn bản thành giọng nói (TTS) và lưu lại thành file mp3
        try:
    ...
    def analyze_word_usage_groq(self, text: str, words: List[str]) -> dict:
        # Nhờ AI phân tích xem các từ vựng mục tiêu có được dùng đúng không
